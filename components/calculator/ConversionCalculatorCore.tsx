...
function SqKmToSqMiCalc() {
    const [sqkm, setSqkm] = useState(10);

    const SQMI_PER_SQKM = 0.386102;
    const sqmi = sqkm * SQMI_PER_SQKM;
    const acres = sqmi * 640;
    const hectares = sqkm * 100;
    const sqm = sqkm * 1000000;
        ...
        <div className="calc-card">
            <div className="calc-field">
                ...
                <input type="range" className="calc-field__slider" min={0.01} max={20000} step={0.01}
                    value={sqkm} onChange={(e) => setSqkm(Number(e.target.value))} />
                <div style={{ display: "flex", gap: "var(--s-2)", alignItems: "center" }}>
                    <input type="number" className="calc-field__input" value={sqkm}
                        onChange={(e) => setSqkm(Number(e.target.value))} style={{ flex: 1 }} />
                    <span className="t-body-sm text-muted">km²</span>
                </div>
            </div>
            ...
            <div style={{ marginTop: "var(--s-4)" }}>
                ...
                <table className="calc-table">
                    <thead>
                        <tr><th>km²</th><th>mi²</th><th>Acres</th><th>Approx. Size</th></tr>
                    </thead>
                    <tbody>
                        {quickRef.map((k) => {
                            const m = k * SQMI_PER_SQKM;
                            const sizeLabel = k <= 0.05 ? "City block" : k <= 0.3 ? "Small park" : k <= 1 ? "Large park" : k <= 3 ? "≈ 1 square mile" : k <= 10 ? "Small town" : k <= 100 ? "Small city" : k <= 1000 ? "Large city" : k <= 5000 ? "US county" : k <= 10000 ? "Small US state" : "Large region";
                            return (
                                <tr key={k} style={k === sqkm ? { background: "var(--n-primary-light)" } : {}}>
                                    <td>{k.toLocaleString()} km²</td>
                                    <td>{m.toLocaleString(undefined, { maximumFractionDigits: 4 })} mi²</td>
                                    <td>{(m * 640).toLocaleString(undefined, { maximumFractionDigits: 0 })} ac</td>
                                    <td>{sizeLabel}</td>
                                </tr>
                            );
                        })}
...
// ─── MWh to kWh ───
function MwhToKwhCalc() {
    const [mwh, setMwh] = useState(1);

    const kwh = mwh * 1000;
    const wh = kwh * 1000;
    const mmbtu = mwh / 0.29307107;
    const gj = mwh * 3.6;
    ...
    return (
        <div className="calc-card">
            <div className="calc-field">
                ...
                <input type="range" className="calc-field__slider" min={0.001} max={1000} step={0.001}
                    value={mwh} onChange={(e) => setMwh(Number(e.target.value))} />
                <div style={{ display: "flex", gap: "var(--s-2)", alignItems: "center" }}>
                    <input type="number" className="calc-field__input" value={mwh}
                        onChange={(e) => setMwh(Number(e.target.value))} style={{ flex: 1 }} />
                    <span className="t-body-sm text-muted">MWh</span>
                </div>
            </div>
            ...
            <div style={{ marginTop: "var(--s-4)" }}>
                ...
                <table className="calc-table">
                    <thead>
                        <tr><th>MWh</th><th>kWh</th><th>GJ</th><th>Context</th></tr>
                    </thead>
                    <tbody>
                        {quickRef.map((m) => {
                            const k = m * 1000;
                            const g = m * 3.6;
                            const ctx = m <= 0.001 ? "LED bulb (1 hour)" : m <= 0.01 ? "EV charge (short trip)" : m <= 0.05 ? "EV full charge (50 kWh)" : m <= 0.1 ? "US home (3 days)" : m <= 0.5 ? "US home (2 weeks)" : m <= 1 ? "Avg US home (1 month)" : m <= 5 ? "Large home (1 month)" : m <= 10 ? "Small commercial" : m <= 100 ? "Large commercial" : "Power plant (1 hour)";
                            return (
                                <tr key={m} style={m === mwh ? { background: "var(--n-primary-light)" } : {}}>
                                    <td>{m} MWh</td>
                                    <td>{k.toLocaleString()} kWh</td>
                                    <td>{g.toFixed(3)} GJ</td>
                                    <td>{ctx}</td>
                                </tr>
                            );
                        })}
...
// ─── L/100km to MPG ───
function Lper100kmToMpgCalc() {
    const [lper100, setLper100] = useState(8);

    const usMpg = 235.215 / lper100;
    const ukMpg = 282.481 / lper100;
        ...
        <div className="calc-card">
            <div className="calc-field">
                ...
                <input type="range" className="calc-field__slider" min={1} max={30} step={0.1}
                    value={lper100} onChange={(e) => setLper100(Number(e.target.value))} />
                <div style={{ display: "flex", gap: "var(--s-2)", alignItems: "center" }}>
                    <input type="number" className="calc-field__input" value={lper100}
                        onChange={(e) => setLper100(Number(e.target.value))} style={{ flex: 1 }} />
                    <span className="t-body-sm text-muted">L/100km</span>
                </div>
            </div>
            ...
            <div style={{ marginTop: "var(--s-4)" }}>
                ...
                <table className="calc-table">
                    <thead>
                        <tr><th>L/100km</th><th>US MPG</th><th>km/L</th><th>Vehicle Type</th></tr>
                    </thead>
                    <tbody>
                        {quickRef.map((l) => {
                            const m = 235.215 / l;
                            const k = 100 / l;
                            const ctx = l <= 3 ? "Electric/hybrid (best)" : l <= 4 ? "Hybrid sedan" : l <= 5 ? "Efficient compact" : l <= 6 ? "Midsize sedan" : l <= 7 ? "Standard sedan" : l <= 8 ? "Crossover / SUV" : l <= 10 ? "Midsize SUV" : l <= 12 ? "Full-size SUV" : l <= 15 ? "Pickup truck" : "Heavy-duty truck";
                            return (
                                <tr key={l} style={l === lper100 ? { background: "var(--n-primary-light)" } : {}}>
                                    <td>{l} L/100km</td>
                                    <td>{m.toFixed(1)} mpg</td>
...
// ─── km/L to MPG ───
function KmPerLToMpgCalc() {
    const [kml, setKml] = useState(12);

    const usMpg = kml * 2.35215;
        ...
        <div className="calc-card">
            <div className="calc-field">
                ...
                <input type="range" className="calc-field__slider" min={1} max={40} step={0.1}
                    value={kml} onChange={(e) => setKml(Number(e.target.value))} />
                <div style={{ display: "flex", gap: "var(--s-2)", alignItems: "center" }}>
                    <input type="number" className="calc-field__input" value={kml}
                        onChange={(e) => setKml(Number(e.target.value))} style={{ flex: 1 }} />
                    <span className="t-body-sm text-muted">km/L</span>
                </div>
            </div>
            ...
            <div style={{ marginTop: "var(--s-4)" }}>
                ...
                <table className="calc-table">
                    <thead>
                        <tr><th>km/L</th><th>US MPG</th><th>L/100km</th><th>Vehicle Type</th></tr>
                    </thead>
                    <tbody>
                        {quickRef.map((k) => {
                            const m = k * 2.35215;
                            const l = 100 / k;
                            const ctx = k <= 5 ? "Heavy truck / old vehicle" : k <= 8 ? "Full-size SUV / pickup" : k <= 10 ? "Midsize SUV" : k <= 12 ? "Standard sedan" : k <= 14 ? "Efficient sedan" : k <= 16 ? "Compact car" : k <= 18 ? "Subcompact" : k <= 20 ? "Hybrid sedan" : k <= 25 ? "Plug-in hybrid" : "Full hybrid / EV-assist";
                            return (
                                <tr key={k} style={k === kml ? { background: "var(--n-primary-light)" } : {}}>
                                    <td>{k} km/L</td>
                                    <td>{m.toFixed(1)} mpg</td>
                                    <td>{l.toFixed(1)} L/100km</td>
                                    <td>{ctx}</td>
...
// ─── MPG to km/L ───
function MpgToKmPerLCalc() {
    const [mpg, setMpg] = useState(30);

    const kml = mpg * 0.42514;
    const lper100 = 100 / kml;
    const ukMpg = mpg * 1.20095;
    ...
    return (
        <div className="calc-card">
            ...
            <div style={{ marginTop: "var(--s-4)" }}>
                ...
                <table className="calc-table">
                    <thead>
                        <tr><th>US MPG</th><th>km/L</th><th>L/100km</th><th>Vehicle Type</th></tr>
                    </thead>
                    <tbody>
                        {quickRef.map((m) => {
                            const k = m * 0.42514;
                            const l = 100 / k;
                            const ctx = m <= 10 ? "Heavy-duty truck / RV" : m <= 15 ? "Full-size SUV / pickup" : m <= 20 ? "Midsize SUV" : m <= 25 ? "Crossover / small SUV" : m <= 30 ? "Standard sedan" : m <= 35 ? "Efficient sedan" : m <= 40 ? "Compact car" : m <= 50 ? "Hybrid sedan" : m <= 60 ? "Plug-in hybrid" : "Full hybrid / EV-assist";
                            return (
                                <tr key={m} style={m === mpg ? { background: "var(--n-primary-light)" } : {}}>
                                    <td>{m} mpg</td>
                                    <td>{k.toFixed(1)} km/L</td>
...
// ─── Ohm to Kiloohm ───
function OhmToKiloohmCalc() {
    const [ohm, setOhm] = useState(4700);

    const kohm = ohm / 1000;
    const mohm = ohm / 1000000;
    const milliohm = ohm * 1000;
    ...
    return (
        <div className="calc-card">
            <div className="calc-field">
                ...
                <input type="range" className="calc-field__slider" min={1} max={100000} step={1}
                    value={ohm} onChange={(e) => setOhm(Number(e.target.value))} />
                <div style={{ display: "flex", gap: "var(--s-2)", alignItems: "center" }}>
                    <input type="number" className="calc-field__input" value={ohm}
                        onChange={(e) => setOhm(Number(e.target.value))} style={{ flex: 1 }} />
                    <span className="t-body-sm text-muted">Ω</span>
                </div>
            </div>
            ...
            <div style={{ marginTop: "var(--s-4)" }}>
                ...
                <table className="calc-table">
                    <thead>
                        <tr><th>Ω</th><th>kΩ</th><th>MΩ</th><th>Common Use</th></tr>
                    </thead>
                    <tbody>
                        {quickRef.map((o) => {
                            const k = o / 1000;
                            const m = o / 1000000;
                            const ctx = o <= 1 ? "Current sense resistor" : o <= 10 ? "LED current limiter (low)" : o <= 47 ? "LED current limiter (5V)" : o <= 100 ? "Signal termination" : o <= 220 ? "LED series (3.3V)" : o <= 470 ? "Logic level shifter" : o <= 1000 ? "General purpose (1kΩ)" : o <= 4700 ? "Pull-up / pull-down (Arduino)" : o <= 10000 ? "Pull-up (I²C bus)" : "Voltage divider (high-Z)";
                            return (
                                <tr key={o} style={o === ohm ? { background: "var(--n-primary-light)" } : {}}>
                                    <td>{o.toLocaleString()} Ω</td>
                                    <td>{k < 1 ? k.toFixed(3) : k.toFixed(1)} kΩ</td>
                                    <td>{m.toFixed(6)} MΩ</td>
                                    <td>{ctx}</td>
                                </tr>
                            );
                        })}
...
function MradToDegCalc() {
    const [mrad, setMrad] = useState(1);

    const deg = mrad / 17.4533;
    const rad = mrad / 1000;
    const natoMil = mrad * (6400 / 6283.2);
    const moa = deg * 60;
    const inchAt100 = mrad * 3.6;
        ...
        <div className="calc-card">
            <div className="calc-field">
                ...
                <input type="range" className="calc-field__slider" min={0} max={100} step={0.01}
                    value={mrad} onChange={(e) => setMrad(Number(e.target.value))} />
                <div style={{ display: "flex", gap: "var(--s-2)", alignItems: "center" }}>
                    <input type="number" className="calc-field__input" value={mrad}
                        onChange={(e) => setMrad(Number(e.target.value))} style={{ flex: 1 }} step={0.01} />
                    <span className="t-body-sm text-muted">mrad</span>
                </div>
            </div>
            ...
            <div style={{ marginTop: "var(--s-4)" }}>
                ...
                <table className="calc-table">
                    <thead>
                        <tr><th>mrad</th><th>Degrees</th><th>MOA</th><th>@ 100yd</th><th>Shooting Context</th></tr>
                    </thead>
                    <tbody>
                        {quickRef.map((m) => {
                            const d = m / 17.4533;
                            const mo = d * 60;
                            const inch = m * 3.6;
                            const ctx = m <= 0.1 ? "1 scope click (0.1 mrad)" : m <= 0.2 ? "2 clicks" : m <= 0.5 ? "Small wind hold" : m <= 1 ? "Standard mil-dot spacing" : m <= 2 ? "200yd zero offset" : m <= 3 ? "300yd holdover" : m <= 5 ? "500yd holdover" : m <= 10 ? "1000yd holdover" : m <= 17.45 ? "= 1 degree" : "Wide angle";
                            return (
                                <tr key={m} style={m === mrad ? { background: "var(--n-primary-light)" } : {}}>
                                    <td>{m} mrad</td>
                                    <td>{d.toFixed(4)}°</td>
                                    <td>{mo.toFixed(2)}</td>
                                    <td>{inch.toFixed(1)}"</td>
                                    <td>{ctx}</td>
                                </tr>
                            );
                        })}
