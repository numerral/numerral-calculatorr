...
function EloMode() {
    ...
    return (
        <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
                <div>
                    ...
                    <input type="number" value={currentRating} onChange={e => setCurrentRating(Number(e.target.value) || 0)}
                        style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid var(--c-border)", fontSize: "1rem", fontWeight: 700 }} />
                </div>
                <div>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                        {[40, 20, 10].map(k => (
                            <button key={k} onClick={() => setKFactor(k)} style={{
                                ...
                                border: kFactor === k ? "2px solid var(--c-primary)" : "1px solid var(--c-border)",
                                background: kFactor === k ? "var(--c-primary-bg, #e8f0fe)" : "var(--c-bg)",
                                fontWeight: kFactor === k ? 800 : 500, fontSize: "0.9rem",
                                color: kFactor === k ? "var(--c-primary)" : "var(--c-text)",
                            }}>K={k}</button>
                        ))}
                    </div>
            ...
            <div style={{ fontSize: "0.78rem", color: "var(--c-text-muted)", marginBottom: 8 }}>Tournament games — add opponents and results:</div>
            <div style={{ overflowX: "auto", maxHeight: 300, overflow: "auto", marginBottom: 12 }}>
                <table style={{ width: "100%", fontSize: "0.82rem", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                        {games.map((g, i) => (
                            <tr key={g.id} style={{ borderBottom: "1px solid var(--c-border)" }}>
                                <td style={{ padding: "4px", fontWeight: 600, color: "var(--c-text-muted)" }}>{i + 1}</td>
                                <td style={{ padding: "4px" }}>
                                    <input type="number" value={g.oppRating} onChange={e => updateGame(g.id, "oppRating", Number(e.target.value) || 0)}
                                        style={{ width: "100%", padding: "6px 8px", borderRadius: 6, border: "1px solid var(--c-border)", fontSize: "0.85rem" }} />
                                </td>
                                    ...
                                    <div style={{ display: "flex", gap: 3, justifyContent: "center" }}>
                                        {([1, 0.5, 0] as Result[]).map(r => (
                                            <button key={r} onClick={() => updateGame(g.id, "result", r)} style={{
                                                ...
                                                border: g.result === r ? "2px solid var(--c-primary)" : "1px solid var(--c-border)",
                                                background: g.result === r ? (r === 1 ? "#dcfce7" : r === 0 ? "#fee2e2" : "#fef3c7") : "var(--c-bg)",
                                                fontWeight: g.result === r ? 700 : 500,
                                                color: r === 1 ? "#16a34a" : r === 0 ? "#dc2626" : "#92400e",
            ...
            {result && (
                <div style={{ background: "var(--c-surface)", borderRadius: 12, padding: "var(--s-4)" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: "var(--s-3)" }}>
                        <div style={{ background: "var(--c-bg)", borderRadius: 8, padding: 12, textAlign: "center" }}>
                            <div style={{ fontSize: "0.72rem", color: "var(--c-text-muted)" }}>New Rating</div>
                            <div style={{ fontSize: "1.5rem", fontWeight: 800, fontFamily: "var(--font-mono)" }}>{result.newRating}</div>
                        </div>
                        <div style={{ background: "var(--c-bg)", borderRadius: 8, padding: 12, textAlign: "center" }}>
                            <div style={{ fontSize: "0.72rem", color: "var(--c-text-muted)" }}>Rating Change</div>
                            <div style={{ fontSize: "1.5rem", fontWeight: 800, color: changeColor, fontFamily: "var(--font-mono)" }}>
                                {result.change > 0 ? "+" : ""}{result.change}
                            </div>
                        </div>
                        <div style={{ background: "var(--c-bg)", borderRadius: 8, padding: 12, textAlign: "center" }}>
                            <div style={{ fontSize: "0.72rem", color: "var(--c-text-muted)" }}>Score</div>
                    ...
                    <details>
                        <summary style={{ cursor: "pointer", fontWeight: 700, fontSize: "0.85rem", color: "var(--c-primary)" }}>📊 Round-by-Round Breakdown</summary>
                            ...
                            <tbody>
                                {result.perGame.map((pg, i) => (
                                    <tr key={i} style={{ borderBottom: "1px solid var(--c-border)" }}>
                                        ...
                                        <td style={{ textAlign: "center", padding: "4px 3px", fontWeight: 700, color: pg.actual === 1 ? "#16a34a" : pg.actual === 0 ? "#dc2626" : "#92400e" }}>
...
function PerfMode() {
    ...
    return (
        <>
            <div style={{ fontSize: "0.78rem", color: "var(--c-text-muted)", marginBottom: 10 }}>Enter opponent ratings and results for each round of your tournament:</div>
            <div style={{ overflowX: "auto", maxHeight: 320, overflow: "auto", marginBottom: 12 }}>
                <table style={{ width: "100%", fontSize: "0.82rem", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                        {games.map((g, i) => (
                            <tr key={g.id} style={{ borderBottom: "1px solid var(--c-border)" }}>
                                <td style={{ padding: "3px", fontWeight: 600, color: "var(--c-text-muted)" }}>{i + 1}</td>
                                <td style={{ padding: "3px" }}>
                                    <input type="number" value={g.oppRating} onChange={e => updateGame(g.id, "oppRating", Number(e.target.value) || 0)}
                                        style={{ width: "100%", padding: "5px 6px", borderRadius: 6, border: "1px solid var(--c-border)", fontSize: "0.82rem" }} />
                                </td>
                                ...
                                <td style={{ textAlign: "center", padding: "3px" }}>
                                    {games.length > 1 && <button onClick={() => removeGame(g.id)} style={{ border: "none", background: "#fee2e2", color: "#dc2626", borderRadius: 6, padding: "3px 8px", cursor: "pointer", fontSize: "0.75rem" }}>✕</button>}
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <button onClick={addGame} style={{ padding: "7px 14px", borderRadius: 8, border: "1px dashed var(--c-primary)", background: "var(--c-primary-bg, #e8f0fe)", color: "var(--c-primary)", fontWeight: 700, cursor: "pointer", fontSize: "0.82rem", marginBottom: 14, width: "100%" }}>+ Add Round</button>

            {result && result.tpr !== null && (
                <div style={{ background: "var(--c-surface)", borderRadius: 12, padding: "var(--s-4)", textAlign: "center" }}>
                    <div style={{ fontSize: "0.78rem", fontWeight: 700, color: "var(--c-primary)", textTransform: "uppercase", letterSpacing: 1 }}>Tournament Performance Rating (TPR)</div>
                    <div style={{ fontSize: "2.6rem", fontWeight: 800, fontFamily: "var(--font-mono)", margin: "8px 0" }}>{result.tpr}</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                        {[
                            ["Avg Opponent", String(result.avgOpp)], ["Score", `${result.totalScore}/${result.totalGames}`], ["Score %", `${(result.pct * 100).toFixed(0)}%`],
                        ].map(([l, v], i) => (
                            <div key={i} style={{ background: "var(--c-bg)", borderRadius: 8, padding: 8, textAlign: "center" }}>
        ...
        <>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
                {(["CM", "FM", "IM", "GM"] as const).map(tt => (
                    <button key={tt} onClick={() => setTarget(tt)} style={{
                        flex: 1, padding: "10px 6px", borderRadius: 8, cursor: "pointer",
                        border: target === tt ? "2px solid var(--c-primary)" : "1px solid var(--c-border)",
                        background: target === tt ? "var(--c-primary-bg, #e8f0fe)" : "var(--c-bg)",
                        fontWeight: target === tt ? 800 : 500, fontSize: "0.85rem",
                        color: target === tt ? "var(--c-primary)" : "var(--c-text)",
                    }}>{tt}</button>
                ))}
            </div>
            ...
            <div style={{ background: "var(--c-surface)", borderRadius: 12, padding: "var(--s-4)" }}>
                <div style={{ textAlign: "center", marginBottom: "var(--s-3)" }}>
                    <div style={{ fontSize: "0.78rem", fontWeight: 700, color: gap === 0 ? "#16a34a" : "var(--c-primary)", textTransform: "uppercase", letterSpacing: 1 }}>
                        {gap === 0 ? `✅ You qualify for ${t.label}!` : `Path to ${t.label} (${target})`}
                    </div>
                    <div style={{ fontSize: "2.2rem", fontWeight: 800, fontFamily: "var(--font-mono)", margin: "8px 0" }}>
                        {gap === 0 ? `${t.rating}+` : `+${gap} points needed`}
                    </div>
                </div>

                <div style={{ background: "var(--c-bg)", borderRadius: 10, height: 24, marginBottom: "var(--s-3)", overflow: "hidden", position: "relative" }}>
                    <div style={{
                        height: "100%", borderRadius: 10, background: gap === 0 ? "#16a34a" : "var(--c-primary)",
                ...
                <table style={{ width: "100%", fontSize: "0.82rem", borderCollapse: "collapse" }}>
                    <tbody>
                        {[
                            ...
                            <tr key={i} style={{ borderBottom: "1px solid var(--c-border)" }}>
                                <td style={{ padding: "6px 4px", color: "var(--c-text-muted)" }}>{l}</td>
...
function ProbMode() {
    ...
    return (
        <>
            ...
            <div style={{ background: "var(--c-surface)", borderRadius: 12, padding: "var(--s-4)", textAlign: "center", marginBottom: "var(--s-3)" }}>
                ...
                <div style={{ fontSize: "2.6rem", fontWeight: 800, fontFamily: "var(--font-mono)", margin: "8px 0" }}>
                    {(es * 100).toFixed(1)}%
                </div>
            ...
            <table style={{ width: "100%", fontSize: "0.78rem", borderCollapse: "collapse" }}>
                ...
                <tbody>
                    {TABLE_DIFFS.map(d => {
                        ...
                        return (
                            <tr key={d} style={{ borderBottom: "1px solid var(--c-border)", background: Math.abs(diff) >= d && Math.abs(diff) < (TABLE_DIFFS[TABLE_DIFFS.indexOf(d) + 1] || 999) ? "var(--c-primary-bg, #e8f0fe)" : undefined }}>
...
export default function FIDERatingCalculatorCore() {
    ...
    return (
        <div style={{ background: "var(--c-card-bg, #fff)", borderRadius: 16, border: "1px solid var(--c-border)", overflow: "hidden", marginBottom: "var(--s-6)" }}>
            <div style={{ padding: "var(--s-4) var(--s-5)", borderBottom: "1px solid var(--c-border)", background: "linear-gradient(135deg, #1e293b, #334155)" }}>
