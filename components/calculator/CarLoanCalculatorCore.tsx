...
export default function CarLoanCalculatorCore() {
    const [mode, setMode] = useState<CalcMode>("emi");

    /* Mode 1: EMI */
    const [carPrice, setCarPrice] = useState(8_00_000);
    ...
    /* Mode 3: Prepayment */
    const [prepayLoan, setPrepayLoan] = useState(7_00_000);
    ...
    /* Mode 4: New vs Used */
    const [cmpPrice, setCmpPrice] = useState(8_00_000);
    const [cmpDown, setCmpDown] = useState(10);
    const [newRate, setNewRate] = useState(8.5);
    const [usedRate, setUsedRate] = useState(12.5);
    const [newTenure, setNewTenure] = useState(7);
    const [usedTenure, setUsedTenure] = useState(5);
    ...
    }, [carPrice, downPct, rate, tenureYrs]);

    const amortRows = useMemo(() => {
        if (!showAmort) return [];
        const loanAmt = carPrice - carPrice * (downPct / 100);
        return calcAmortization(loanAmt, rate, tenureYrs * 12);
    }, [carPrice, downPct, rate, tenureYrs, showAmort]);
    ...
    const eligResult = useMemo(() => {
        ...
        return { maxEMI, maxLoan, maxCar90: maxLoan / 0.90, maxCar80: maxLoan / 0.80 };
    ...
    const cmpResult = useMemo(() => {
        ...
        const N1 = newTenure * 12, N2 = usedTenure * 12;
        const emi1 = calcEMI(loanAmt, newRate, N1);
        const emi2 = calcEMI(loanAmt, usedRate, N2);
    ...
    }, [cmpPrice, cmpDown, newRate, usedRate, newTenure, usedTenure]);
    ...
    return (
        <div className="con-calc" style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
            <h3 className="con-calc__title">🚗 Car Loan EMI Calculator</h3>

            <div style={{ display: "flex", gap: "8px", marginBottom: "20px", flexWrap: "wrap" }}>
                {([["emi", "EMI Calculator"], ["eligibility", "Loan Eligibility"], ["prepay", "Prepayment Impact"], ["compare", "New vs Used"]] as [CalcMode, string][]).map(([m, label]) => (
                ...
                <>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
                        <div className="con-input">
                            <label className="con-input__label">Car On-Road Price <span className="con-input__unit">(₹)</span></label>
                            <input type="number" className="con-input__field" value={carPrice} onChange={(e) => setCarPrice(+e.target.value)} min={50000} step={50000} />
                            <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "4px" }}>{lakhLabel(carPrice)}</div>
                        </div>
                        <div className="con-input">
                            ...
                            <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "4px" }}>{formatINR(carPrice * (downPct / 100))}</div>
                        </div>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
                        <div className="con-input">
                            ...
                            <input type="number" className="con-input__field" value={rate} onChange={(e) => setRate(+e.target.value)} min={5} max={20} step={0.01} />
                        </div>
                        <div className="con-input">
                            <label className="con-input__label">Loan Tenure <span className="con-input__unit">(Years)</span></label>
                            <input type="number" className="con-input__field" value={tenureYrs} onChange={(e) => setTenureYrs(+e.target.value)} min={1} max={8} />
                        </div>
                    </div>
                    ...
                    {emiResult.emi > 0 && (
                        <div className="con-calc__results">
                            <h4>EMI Results</h4>
                            <div className="con-result-row"><span className="con-result-row__label">Car On-Road Price</span><span className="con-result-row__value">{formatINR(carPrice)}</span></div>
            ...
            {mode === "eligibility" && (
                <>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "16px" }}>
                        <div className="con-input">
                            <label className="con-input__label">Interest Rate <span className="con-input__unit">(% p.a.)</span></label>
                            <input type="number" className="con-input__field" value={eligRate} onChange={(e) => setEligRate(+e.target.value)} min={5} max={20} step={0.01} />
                        </div>
                        <div className="con-input">
                            <label className="con-input__label">Loan Tenure <span className="con-input__unit">(Years)</span></label>
                            <input type="number" className="con-input__field" value={eligTenure} onChange={(e) => setEligTenure(+e.target.value)} min={1} max={8} />
                        </div>
                    </div>
                    ...
                    {eligResult && (
                        <div className="con-calc__results">
                            ...
                            <div className="con-result-row"><span className="con-result-row__label">Max Car Price (10% down / 90% LTV)</span><span className="con-result-row__value" style={{ fontWeight: 700 }}>{formatINR(eligResult.maxCar90)}</span></div>
                            <div className="con-result-row"><span className="con-result-row__label">Max Car Price (20% down / 80% LTV)</span><span className="con-result-row__value">{formatINR(eligResult.maxCar80)}</span></div>

                            <div className="explanation__highlight" style={{ marginTop: "16px", fontSize: "0.85rem" }}>
                                <strong>FOIR Rule:</strong> Banks allow max 50% of net income towards all EMIs. With {formatINR(monthlySalary)} salary, your max new EMI is <strong>{formatINR(eligResult.maxEMI)}</strong>. At {eligRate}% for {eligTenure} years, you can borrow <strong>{formatINR(eligResult.maxLoan)}</strong>. With 10% down payment, you can afford a car up to <strong>{formatINR(eligResult.maxCar90)}</strong>.
                            </div>
                        </div>
                    )}
                </>
                ...
                <>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginBottom: "16px" }}>
                        <div className="con-input">
                            ...
                            <input type="number" className="con-input__field" value={prepayLoan} onChange={(e) => setPrepayLoan(+e.target.value)} min={10000} step={10000} />
                            <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "4px" }}>{lakhLabel(prepayLoan)}</div>
                        </div>
                        <div className="con-input">
                            <label className="con-input__label">Interest Rate <span className="con-input__unit">(% p.a.)</span></label>
                            <input type="number" className="con-input__field" value={prepayRate} onChange={(e) => setPrepayRate(+e.target.value)} min={5} max={20} step={0.01} />
                        </div>
                        <div className="con-input">
                            <label className="con-input__label">Tenure <span className="con-input__unit">(Years)</span></label>
                            <input type="number" className="con-input__field" value={prepayTenure} onChange={(e) => setPrepayTenure(+e.target.value)} min={1} max={8} />
                        </div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "16px" }}>
                        <div className="con-input">
                    ...
                    <div className="con-calc__results">
                        ...
                        <div className="explanation__highlight" style={{ marginTop: "16px", fontSize: "0.85rem" }}>
                            <strong>RBI Rule (Jan 2026):</strong> Banks cannot charge prepayment/foreclosure penalty on <strong>floating-rate car loans</strong> for individual borrowers. If your car loan is floating rate, prepayment costs <strong>₹0 in penalty</strong>. Fixed rate loans may carry 2–5% penalty.
                        </div>
                    </div>
                </>
            )}
            ...
            {mode === "compare" && (
                <>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "16px" }}>
                        <div className="con-input">
                            <label className="con-input__label">Car Price <span className="con-input__unit">(₹)</span></label>
                            <input type="number" className="con-input__field" value={cmpPrice} onChange={(e) => setCmpPrice(+e.target.value)} min={50000} step={50000} />
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "16px" }}>
                        <div style={{ padding: "16px", border: "2px solid #16a34a", borderRadius: "12px", background: "rgba(22,163,74,0.03)" }}>
                            <div style={{ fontWeight: 700, marginBottom: "12px", color: "#16a34a", fontSize: "0.85rem", textTransform: "uppercase", letterSpacing: "0.5px" }}>🆕 New Car Loan</div>
                            <div className="con-input" style={{ marginBottom: "8px" }}>
                                <label className="con-input__label">Interest Rate <span className="con-input__unit">(% p.a.)</span></label>
                                <input type="number" className="con-input__field" value={newRate} onChange={(e) => setNewRate(+e.target.value)} min={5} max={20} step={0.01} />
                            </div>
                            <div className="con-input">
                                <label className="con-input__label">Max Tenure <span className="con-input__unit">(Years)</span></label>
                                <input type="number" className="con-input__field" value={newTenure} onChange={(e) => setNewTenure(+e.target.value)} min={1} max={8} />
                            </div>
                        </div>
                        <div style={{ padding: "16px", border: "2px solid #d4620a", borderRadius: "12px", background: "rgba(212,98,10,0.03)" }}>
                            <div style={{ fontWeight: 700, marginBottom: "12px", color: "#d4620a", fontSize: "0.85rem", textTransform: "uppercase", letterSpacing: "0.5px" }}>🔄 Used Car Loan</div>
                            <div className="con-input" style={{ marginBottom: "8px" }}>
                                <label className="con-input__label">Interest Rate <span className="con-input__unit">(% p.a.)</span></label>
                                <input type="number" className="con-input__field" value={usedRate} onChange={(e) => setUsedRate(+e.target.value)} min={5} max={20} step={0.01} />
                            </div>
                            <div className="con-input">
                                <label className="con-input__label">Max Tenure <span className="con-input__unit">(Years)</span></label>
                                <input type="number" className="con-input__field" value={usedTenure} onChange={(e) => setUsedTenure(+e.target.value)} min={1} max={8} />
                            </div>
                        </div>
                    </div>

                    <div className="con-calc__results">
                        <h4>New vs Used Car Comparison</h4>
                        <div style={{ overflowX: "auto" }}>
                            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem", borderRadius: "8px", overflow: "hidden", border: "1px solid var(--border)" }}>
                                <thead><tr style={{ background: "#d4620a", color: "#fff" }}>
                                    <th style={{ ...thStyle, textAlign: "left" }}>Parameter</th>
                                    <th style={{ ...thStyle, textAlign: "right" }}>🆕 New ({newRate}%, {newTenure}yr)</th>
                                    <th style={{ ...thStyle, textAlign: "right" }}>🔄 Used ({usedRate}%, {usedTenure}yr)</th>
                                </tr></thead>
                                <tbody>
                        ...
                        <div className="explanation__highlight" style={{ marginTop: "16px", fontSize: "0.85rem" }}>
                            <strong>Key Insight:</strong> The used car loan at {usedRate}% costs <strong>{formatINR(cmpResult.diff)} more in interest</strong> than the new car loan at {newRate}%. Even though used cars cost less upfront, the higher interest rate and shorter tenure can make total borrowing cost comparable. Always compare total cost of ownership including insurance, maintenance, and depreciation.
                        </div>
                    </div>
                </>
            )}
