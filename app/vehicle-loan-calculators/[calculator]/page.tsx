...
export default async function VehicleCalculatorHubPage({ params }: PageProps) {
    ...
    const schemaData = JSON.stringify([breadcrumbSchema([{ name: "Home", url: `${SITE_URL}/` }, { name: "Vehicle Loan Calculators", url: canonicalUrl("/vehicle-loan-calculators") }, { name: calc.title }]), webAppSchema(calc.title, pageUrl)]);
