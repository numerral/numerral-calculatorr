...
export default async function ElectricalCalculatorHubPage({ params }: PageProps) {
    ...
    const schemaData = JSON.stringify([breadcrumbSchema([{ name: "Home", url: `${SITE_URL}/` }, { name: "Electrical Calculators", url: canonicalUrl("/electrical-calculators") }, { name: calc.title }]), webAppSchema(calc.title, pageUrl)]);
